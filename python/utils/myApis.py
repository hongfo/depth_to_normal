...
lap_ker_beta = np.array([[-1, -1, -1], [-1, 8, -1], [-1, -1, -1]])

lap_ker_gamma = np.array([[0.25, 0.5, 0.25], [0.5, -3, 0.5], [0.25, 0.5, 0.25]])
...
def MRF_optim(depth, n_est, lap_conf="DLF-alpha"):
    ...
    if lap_conf == "1D-DLF":
        ...
        Z_laplace_stack = np.array(
            (
                np.hstack((np.inf * np.ones((h, 1)), Z_laplace_hor[:, :-1])),
